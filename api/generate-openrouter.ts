...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        if (!openrouterApiKey) {
            console.error("OpenRouter API key not set in environment");
            return res.status(500).json({ error: 'Server configuration error: OpenRouter API key missing.' });
        }

        // 4. Convert history to OpenAI-compatible format
        const systemInstruction = getSystemInstruction(fileSystem);
        const openRouterHistory = history.map((h: any) => {
            const content: any[] = [];
