...
import { 
    ...
    PencilIcon, HammerIcon, WrenchIcon, ChevronRight, CheckCircle2Icon, 
...
const getTaskStatusDetails = (status: TaskStatus): { icon: React.ReactNode, color: string } => {
  switch (status) {
    ...
    case 'COMPLETED':
      return { icon: <CheckCircle2Icon className="w-5 h-5" />, color: 'text-green-500 dark:text-green-400' };
    case 'FAILED':
...
const getActionDetails = (action: AIAction): { icon: React.ReactNode, text: React.ReactNode, pill?: string } => {
  switch (action.type) {
    ...
    case 'EDIT':
      return { icon: <PencilIcon className="w-5 h-5 text-blue-500 dark:text-blue-300" />, text: 'Edited', pill: action.target };
    case 'BUILD':
    ...
    case 'COMMAND':
      return { icon: <ChevronRight className="w-5 h-5 text-gray-300 dark:text-gray-200" />, text: 'Run', pill: action.target };
    case 'COMPLETE':
      return { icon: <CheckCircle2Icon className="w-5 h-5 text-green-500 dark:text-green-400" />, text: 'Task complete' };
    case 'TASK_START':
    case 'TASK_COMPLETE':
    case 'TASK_FAIL':
...
const TaskJournal: React.FC<{ tasks: AITask[], isLoading: boolean }> = ({ tasks, isLoading }) => {
  ...
  const renderHeader = () => (
    <div className="flex items-center text-sm mb-3">
        {isLoading ? <SpinnerIcon className="w-5 h-5 text-gray-300 dark:text-gray-300 animate-spin" /> : <CheckCircle2Icon className="w-5 h-5 text-green-500 dark:text-green-400"/>}
...
const ActionRow: React.FC<{ action: AIAction }> = ({ action }) => {
    ...
    return (
        <div className="flex items-center text-sm mb-2 group">
            ...
            {hasLink && <a href="#" className="text-blue-400 text-xs opacity-0 group-hover:opacity-100 transition-opacity">Open &gt;</a>}
        </div>
    );
};
...
// FIX: Define the AIActionJournalProps interface to fix the "Cannot find name" error.
